...
# Global variables
# -------------------------
COOKIE_FILE_PATH = '/home/tompots/PycharmProjects/insta_promoter_core/application_folder/session_cookies.pkl'  # File path to save cookies
CHROMEDRIVER_PATH = os.getenv("CHROMEDRIVER_PATH",
                              "/usr/bin/chromedriver")  # Fetches ChromeDriver path from env or sets default
binary_location = "/usr/bin/google-chrome"
PROXY_FILE_PATH = "/home/tompots/PycharmProjects/insta_promoter_core/data/proxies.csv"  # File path for the proxy list
...
def lanuch_initialize_driver():
    ...
    def load_extensions(chrome_options, directories):
        ...
        for extension_dir in directories:
            if os.path.exists(extension_dir) and os.path.isdir(extension_dir):
                ...
                if os.path.exists(manifest_path):
                    valid_extensions.append(extension_dir)
        ...
        # Combine all valid extensions into a comma-separated string and add them as a single argument
        if valid_extensions:
            chrome_options.add_argument(f"--load-extension={','.join(valid_extensions)}")
        else:
            logging.warning("No valid extensions found to load.")
    ...
    extensions_directories = [
        "/home/tompots/PycharmProjects/insta_promoter_core/data/automation_security",
        "/home/tompots/PycharmProjects/insta_promoter_core/data/csp_bypass _extension",
    ]
