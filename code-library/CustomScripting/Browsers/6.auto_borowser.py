...
def lanuch_initialize_driver():
    ...
    try:
        ...
        if not os.path.exists(PROXY_FILE_PATH) or os.stat(PROXY_FILE_PATH).st_size == 0:
            logging.warning("Proxy file is blank or doesn't exist. Skipping proxy setup.")

        else:
            if global_proxy_row < len(global_proxy_list):  # Check if there are proxies available
