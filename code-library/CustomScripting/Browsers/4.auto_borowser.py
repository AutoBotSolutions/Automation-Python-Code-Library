...
from selenium import webdriver  # Selenium WebDriver for automation
from selenium.webdriver.chrome.options import Options  # Chrome options customization
from selenium.webdriver.chrome.service import Service  # Service management for ChromeDriver
from os_get_installed_useragent import get_chrome_user_agent  # Fetch a dynamic user-agent string for the browser
import requests  # For direct proxy connectivity testing
...
proxy_test_urls = [  # Additional fallback URLs for proxy testing
...
def verify_proxy(driver_instance, proxy_ip):
    ...
    try:
        ...
        for test_url in proxy_test_urls:  # Attempt multiple verification endpoints
            try:
                ...
                if response.get("origin", "").startswith(proxy_ip):
                    logging.debug(f"Proxy {proxy_ip} verification succeeded via {test_url}.")
                    return True
            except Exception as e:
                logging.warning(f"Proxy verification failed using {test_url}: {e}. Trying next URL.")
...
# --------------------------
# Function to verify proxy directly using requests
# --------------------------
def test_proxy_connectivity(proxy_ip, proxy_port):
    ...
    try:
        ...
        logging.debug(f"Direct proxy connectivity failed for {proxy_ip}:{proxy_port}. Error: {e}")
        return False
...
def lanuch_initialize_driver():
    ...
    chrome_options.add_argument(f"user-agent={get_chrome_user_agent()}")  # Dynamic User-Agent
